    ...
    for option in options:
        match option:
            ...
            case 4:
                ...
                axes.set_xlabel("Test size")
                axes.set_ylabel("Time of execution")
                axes.set_title(f"Topological sort: Tarjan")
                for representation in representations:
                    print(f"Starting representation: {representation}")
                    times = {}
                    ...
                    for test_size in scale:
                        ...
                        match representation:
                            case "adj_matrix":
                                g =create_adjacency_matrix(test_size)
                                start = time()
                                topological_sort_tarjan(g,test_size,1)
                                end = time()
                            case "edge_table":
                                g = create_edge_table(test_size)
                                start = time()
                                topological_sort_tarjan(g,test_size,3)
                                end = time()
                            case "successor_list":
                                g = create_adjacency_matrix(test_size)
                                g = create_successor_list(g)
                                start = time()
                                topological_sort_tarjan(g,test_size,2)
                                end = time()
                        times[representation].append(end - start)
